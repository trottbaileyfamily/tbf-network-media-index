...
class TBFNMI_Indexer_Admin {
  ...
  public static function assets($hook) {
    ...
    wp_localize_script('tbfnmi-indexer-admin', 'TBFNMI_INDEXER', [
  ...
  public static function get_state() {
    ...
    $opt = is_multisite() ? get_site_option('tbfnmi_indexer_state', []) : get_option('tbfnmi_indexer_state', []);
    if ( ! is_array($opt) ) $opt = [];
  ...
  public static function save_state(array $state) {
    ...
    if ( is_multisite() ) update_site_option('tbfnmi_indexer_state', $state);
    else update_option('tbfnmi_indexer_state', $state);
  }
  ...
  public static function render() {
    ...
    echo '<div class="wrap tbfnmi-indexer-wrap">';
    echo '<h1>Photofall Index</h1>';
    echo '<p>Build the fast media index used by <code>/photo/</code> and sitemaps.</p>';

    echo '<div class="tbfnmi-indexer-card">';
    echo '<h2>Status</h2>';
    echo '<div class="tbfnmi-indexer-status">';
    ...
    echo '<div class="tbfnmi-indexer-actions">';
    ...
    echo '<pre class="tbfnmi-indexer-log" style="max-height:280px; overflow:auto; background:#111; color:#c7f0d8; padding:12px; border-radius:8px;"></pre>';
    echo '</div></div>';
  }
}
