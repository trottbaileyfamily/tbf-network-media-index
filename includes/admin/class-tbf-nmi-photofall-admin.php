<?php
/**
 * File: includes/admin/class-tbf-nmi-photofall-admin.php
 * Version: 4.0.0
 */
if ( ! defined('ABSPATH') ) exit;

class TBF_NMI_PhotoFall_Admin {
  ...
  public static function assets($hook) {
    ...
    wp_enqueue_script('tbf-photofall-admin', TBF_NMI_URL . 'assets/js/photofall-admin.js', ['jquery'], TBF_NMI_VER, true);
  }

  private static function get_settings() {
    $raw = is_multisite() ? get_site_option('tbf_nmi_settings', []) : get_option('tbf_nmi_settings', []);
    return is_array($raw) ? $raw : [];
  }

  private static function save_settings(array $s) {
    if ( is_multisite() ) update_site_option('tbf_nmi_settings', $s);
    else update_option('tbf_nmi_settings', $s);
  }

  public static function render() {
    if ( ! current_user_can(is_multisite() ? 'manage_network_options' : 'manage_options') ) wp_die('Permission denied');

    $settings = class_exists('TBF_NMI_Plugin') ? TBF_NMI_Plugin::instance()->get_settings() : self::get_settings();
    ...
    $photoUrl = home_url('/' . trim(TBF_NMI_PHOTOFALL_BASE, '/') . '/');
