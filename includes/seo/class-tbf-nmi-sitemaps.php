<?php
/**
 * File: includes/seo/class-tbf-nmi-sitemaps.php
 * Version: 4.0.0
 */
if ( ! defined('ABSPATH') ) exit;

class TBF_NMI_Sitemaps {
  ...
  public static function rewrite_rules() {
    add_rewrite_rule('^photo-sitemap-index\.xml$', 'index.php?tbf_nmi_sm=photo_index', 'top');
    add_rewrite_rule('^photo-sitemap-([0-9]{1,6})\.xml$', 'index.php?tbf_nmi_sm=photo&tbf_nmi_sm_page=$matches[1]', 'top');
    add_rewrite_rule('^video-sitemap-index\.xml$', 'index.php?tbf_nmi_sm=video_index', 'top');
    add_rewrite_rule('^video-sitemap-([0-9]{1,6})\.xml$', 'index.php?tbf_nmi_sm=video&tbf_nmi_sm_page=$matches[1]', 'top');
  }

  public static function query_vars($vars) {
    $vars[] = 'tbf_nmi_sm';
    $vars[] = 'tbf_nmi_sm_page';
    return $vars;
  }

  public static function maybe_render() {
    $kind = sanitize_key((string)get_query_var('tbf_nmi_sm'));
    if ( ! $kind ) return;
    ...
    if ( $kind === 'photo' ) { self::render_chunk('photo', max(1,(int)get_query_var('tbf_nmi_sm_page'))); exit; }
    if ( $kind === 'video' ) { self::render_chunk('video', max(1,(int)get_query_var('tbf_nmi_sm_page'))); exit; }
